...
describe('EventThrottle', function Test() {
  ...
  describe('register()', () => {
    describe('when unregistered', () => {
      ...
      it('no event is published', () => assert.ok(!this.window.timeout))
    ...
    describe('when registered', () => {
      ...
      it('and not scrolled, no event is published', () => assert.ok(!this.window.timeout))

      it('and scrolled, no event is published', () => {
      ...
      describe('and scrolled', () => {
        ...
        it('and a timeout, an event is published', done => {
          this.window.dispatchEvent = () => done()
          this.window.timeout()
        })

        it('and a timeout, one event is published', () => {
        ...
        describe('and deregistered and registered', () => {
          ...
          it('no event is published', () => assert.ok(this.window.sets === 1))
  ...
  describe('deregister()', () => {
    describe('when registered', () => {
      ...
      describe('and scrolled and deregistered', () => {
        ...
        it('unregistered is reported', () => assert.ok(!this.eventThrottle.registered()))
        it('no event is published', () => assert.ok(this.window.sets === this.window.clears))
        it('no event is subscribed', () => assert.ok(!this.window.scroll))
      })
    })
  })
